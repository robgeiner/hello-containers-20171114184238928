module "target_group" {
  ...
  name = "${var.ENVIRONMENT}-${data.terraform_remote_state.cluster.cluster_short_name}-${data.terraform_remote_state.service-registry.hulu_name}"
  project = "${var.PROJECT}"
  vpc_id = "${data.terraform_remote_state.vpc.vpc_id}"
  healthcheck_path = "${data.terraform_remote_state.service-registry.hulu_healthcheck_path}"
  healthcheck_protocol = "${data.terraform_remote_state.service-registry.hulu_healthcheck_protocol}"
}

resource "aws_alb_listener_rule" "hulu" {
  listener_arn = "${data.terraform_remote_state.cluster.listener_public_arn}"
  priority     = "${data.terraform_remote_state.service-registry.hulu_priority}"
  action {
    type             = "forward"
  ...
  condition {
    field  = "path-pattern"
    values = ["${data.terraform_remote_state.service-registry.hulu_path}${data.terraform_remote_state.service-registry.hulu_api_key}*"]
  }
}
...
data "template_file" "task_template" {
    ...
    vars {
        ...
        app_name = "${data.terraform_remote_state.service-registry.hulu_name}"
        app_version = "${var.version}"
        api_key = "${data.terraform_remote_state.service-registry.hulu_api_key}"
        ...
        newrelic_app_name = "${data.terraform_remote_state.service-registry.hulu_name}-${var.ENVIRONMENT}"
        ...
        twitter_callback_url = "https://ui.${data.terraform_remote_state.route53.domain}${data.terraform_remote_state.service-registry.hulu_path}${data.terraform_remote_state.service-registry.hulu_api_key}/auth/twitter/callback"
    }
}

resource "aws_ecs_task_definition" "task_definition" {
    family = "${data.terraform_remote_state.service-registry.hulu_name}-${var.ENVIRONMENT}"
...
resource "aws_ecs_service" "service" {
    name = "${data.terraform_remote_state.service-registry.hulu_name}-${var.ENVIRONMENT}"
    ...
    load_balancer {
        target_group_arn = "${module.target_group.arn}"
        container_name = "${data.terraform_remote_state.service-registry.hulu_name}"
